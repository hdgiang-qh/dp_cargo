...
class BarCodeReceive extends StatefulWidget {
  const BarCodeReceive({super.key});

  @override
  State<BarCodeReceive> createState() => _BarCodeState();
}

class _BarCodeState extends State<BarCodeReceive> {
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) async {
      ...
      final isStatus = deliveryBillDetailController.vnDeliveryBoxes
          .where((element) => element.status == BoxesStatus.newCode)
          .toSet()
          .toList();
      ...
      if (scannedCode?.isNotEmpty ?? false) {
        ...
        if (isScannedCodeExist) {
          if (!isCheck) {
            AppSnackBar.playSound("assets/sounds/sound.mp3");
            isCheck = true;
